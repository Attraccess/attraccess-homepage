...
const plans: { key: string; items: number; highlighted?: boolean; badge?: boolean; green?: boolean }[] = [
  { key: "nonprofit", items: 4, green: true },
  { key: "community", items: 4, highlighted: true, badge: true },
  { key: "standard", items: 4 },
];
...
export function PricingTiers() {
  ...
  return (
    <section id="pricing" className="relative overflow-hidden bg-muted px-6 py-28 sm:px-8 noise-overlay">
      <div className="relative mx-auto max-w-[1200px]">
        ...
        <div className="mt-14 grid items-stretch gap-6 md:grid-cols-3">
          {plans.map(({ key, items, highlighted, badge, green }, i) => (
            <Reveal
              key={key}
              delay={i * 80}
              className={cn(
            ...
            >
              ...
              <div className="mt-4 flex items-baseline gap-1.5">
                <span className="font-display text-[44px] font-bold tracking-tight text-foreground">
                  {t(`home.pricing.${key}.price`)}
                </span>
